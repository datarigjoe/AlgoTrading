...
#         if order_response.get("status") == "success" and "data" in order_response:
#             order_id = order_response["data"].get("orderid", "N/A")
...
#             print("Error: API returned a string instead of a dictionary.")
#             return None
...
def log_order(timestamp, token, trade_type, price, order_id):
    """Log successfully placed orders to a CSV file."""
    ...
    with open(csv_file, mode='a', newline='') as file:
        writer = csv.writer(file)
        if not file_exists:
            writer.writerow(["Timestamp", "Token", "Trade Type", "Price", "Order ID"])
        writer.writerow([timestamp, token, trade_type, price, order_id])
